...
Beginning in Amplify CLI version 8.5.0, there is a feature flag named `enableautoindexquerynames`. If enabled, this flag allows configuration of secondary indexes without any explicit naming.

For more information on feature flags, refer to the [feature flags reference](/cli/reference/feature-flags/).
...
The example client query below allows you to query for "Customer" based on their `accountRepresentativeID` using auto-generated index and query names:
...
To opt out of query generation for these indexes, setting the `queryField` parameter to `null` will disable the query in the generated schema.
